...
- 在学生评教页面，加入自动评教功能。用户可以通过操控前端控制台来对评教进行批量操作。
- 支持评教、评教后保存、自定义分数评教、手动查看、自动提交（默认为最高分策略）
- 手动模式：允许用户手动调整选项，提供了四个不同分数的预设选项供用户快速操作。页面上可显示各选项对应分数及总分
- 自动模式：根据默认策略，自动选择最高分进行评教，并避开同一选项。

![自动评教](./docs/static/eval_submit.png)

![页面增强](./docs/static/eval_usageinfo.png)
